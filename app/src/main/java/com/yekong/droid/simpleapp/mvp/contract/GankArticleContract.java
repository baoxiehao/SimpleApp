...
public interface GankArticleContract {

    interface View extends BaseView<List<Gank.Article>> {
    }

    class Presenter extends BasePagePresenter<View, List<Gank.Article>> {

        private int mMonth;
        private int mDay;

        private List<String> mPages = new ArrayList<>();
        List<Gank.Article> mData;
        private Map<String, List<Gank.Article>> mPageDataMap = new HashMap<>();

        void getAndroid(int month, int day) {
            SimpleApp.getAppComponent().getGankService().getArticle("Android", month, day)
        ...
        @Override
        protected void onPageData(String page, List<Gank.Article> pageData) {
